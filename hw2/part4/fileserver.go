...
func main() {
	...
	GOOS=darwin  go build fileserver.go // for MacOS
	*/
	...
	// Modify The below function to start serving TLS
	log.Fatal(http.ListenAndServe(":"+*port, http.FileServer(http.Dir(*path)))) // Hint: golang has excellent documentation
}
